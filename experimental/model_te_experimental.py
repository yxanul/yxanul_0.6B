#!/usr/bin/env python3
"""
TransformerEngine FP8 version with EXPERIMENTAL corrected attention.
Fixes shape layout and GQA handling for DotProductAttention.
This version properly handles [B, S, H, D] format for FP8 attention.

To debug FP8 attention backend issues, set environment variables:
  export NVTE_FUSED_ATTN_BACKEND=1  # 0=math, 1=FlashAttn, 2=cuDNN
  export NVTE_DEBUG=1 NVTE_DEBUG_LEVEL=1
"""

import os
# Try FlashAttention backend first (better consumer GPU support)
os.environ.setdefault('NVTE_FUSED_ATTN_BACKEND', '1')  # Force FlashAttention backend
os.environ.setdefault('NVTE_DEBUG', '0')  # Set to '1' for debugging
...
def get_fp8_recipe(config, use_mx=None):
    ...
    if use_mx and MXFP8_AVAILABLE:
        ...
        print("Using standard DelayedScaling FP8 with EXPERIMENTAL FP8 attention")
        print(f"  Backend: {os.environ.get('NVTE_FUSED_ATTN_BACKEND', 'auto')} (0=math, 1=FlashAttn, 2=cuDNN)")
        print("  Testing corrected shape layout for DotProductAttention")
        return DelayedScaling(
            fp8_format=Format.HYBRID,  # E4M3 forward, E5M2 backward
            amax_history_len=config.fp8_amax_history_len,
            amax_compute_algo=config.fp8_amax_compute_algo,
            fp8_dpa=True,  # EXPERIMENTAL: Enable FP8 attention with correct shapes
        )
...
class Attention(nn.Module):
    """Multi-Head Attention with GQA - EXPERIMENTAL FP8 attention with correct shapes."""
    ...
    def __init__(self, config):
        ...
        # EXPERIMENTAL: Create DotProductAttention for FP8 attention
        self.use_fp8_attention = True  # Flag to enable/disable
        if self.use_fp8_attention:
            try:
                ...
                print("  DotProductAttention module created successfully")
            except Exception as e:
                print(f"  Warning: Could not create DPA module: {e}")
                self.use_fp8_attention = False
    
    def forward(self, x, rope_cache):
        B, T, C = x.shape
        
        # QKV projections (FP8 via TE Linear)
        q = self.q_proj(x).view(B, T, self.n_head, self.head_dim)
        k = self.k_proj(x).view(B, T, self.n_kv_heads, self.head_dim)
        v = self.v_proj(x).view(B, T, self.n_kv_heads, self.head_dim)
        
        # Apply RoPE (operates on [B, T, H, D] directly)
        ...
        if self.use_fp8_attention and hasattr(self, 'dpa'):
            ...
            # DPA expects [B, S, H, D] - we already have this!
            # No transpose needed - this was the bug!
            try:
